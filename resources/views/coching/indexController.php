...
class IndexController extends Controller
{
    ...
    public function Home_index()
     {
        $country = DB::table('country')->where('Is_active', 'Yes')->orderBy('name', 'ASC')->get();
        foreach($country as $country_value){
           
          $country_id = $country_value->id;
          $institute_list = DB::table('institute_list')->where('Is_active', 'Yes')->where('institute_list.country_id',$country_id)->select('institute_list.*')->get();

                  foreach($institute_list as $institute_list_value){

                      $ins_id = $institute_list_value->id;
              }
            $data['cour'] = DB::table('courses')->where('Is_active', 'Yes')->where('courses.intakes',$ins_id)->select('courses.*')->get();
        }

        $data['institute_list'] = $institute_list;
...
// $data['country']= DB::table('country')->where('Is_active', 'Yes')->orderBy('name', 'ASC')->get();
         ...
         // institute data
        // $data['institute_country'] = DB::table('institute_list')->leftJoin('country','country.id','institute_list.country_id')->where('institute_list.Is_active', 'Yes')->select('institute_list.*','country.name')
        //     ->get();
                        //   $country_id_Varible= $data['institute_country'][0];
                        //   $country_ids = [];
                        //   foreach ($country_id_Varible as $key => $value) {

                        //  $country_val = $value->country_id;
                        //  $country_data = array_push($country_ids,$country_val);
                        // } 
                        //   dd($country_data);
            // $country_ids = [];
            // foreach ($institute_countries as $key => $value) {

            //     // $value->my_cutom_couln = $data['country'];
            //     // unset($value->ins_name);
            ...
            //     foreach ($value->my_cutom_couln as $k => $v) {
            //         $v->ins_nam = $value->ins_name;
            //         array_push($final_data, $v);
            //     }
            // }


       // $country_id =  $data['institute_country']->country_id ;
    // $data['course_data'] = DB::table('courses')->leftjoin('institute_list', 'institute_list.id', 'courses.intakes')->whereIn('institute_list.country_id', $country_ids )->where('institute_list.Is_active', 'Yes')->select('courses.*','institute_list.ins_name','institute_list.ilogo')->get();

   // dd($data['course_data'], $country_ids);
     // $data['course_data'] = DB::table('courses')->join('institute_list','institute_list.id','courses.intakes')->leftJoin('country','country.id','institute_list.country_id')->where('institute_list.Is_active', 'Yes')->select('courses.*','institute_list.ins_name','institute_list.ilogo','country.name')->limit(2)->get();
        // $data['course_data'] = DB::table('courses')->join('institute_list','institute_list.id','courses.intakes')->leftJoin('country','country.id','institute_list.country_id')->where('institute_list.Is_active', 'Yes')->where('institute_list.country_id','==','country.id')->select('courses.*','institute_list.ins_name','institute_list.ilogo','country.name')->limit(2)->get();
        // dd($data_cour);
